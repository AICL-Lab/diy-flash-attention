...
class TestFlashAttentionErrorHandling:
    ...
    def test_seq_lens_invalid_values(self):
        ...
        with pytest.raises(ValueError, match="positive"):
            flash_attention(
                q, k, v, seq_lens=torch.tensor([0, 64], device="cuda", dtype=torch.int32)
            )

        with pytest.raises(ValueError, match="seq_len"):
            flash_attention(
                q, k, v, seq_lens=torch.tensor([129, 128], device="cuda", dtype=torch.int32)
            )
    ...
    def test_seq_lens_3d_length_mismatch(self):
        ...
        with pytest.raises(ValueError, match="length 1"):
            flash_attention(
                q, k, v, seq_lens=torch.tensor([64, 64], device="cuda", dtype=torch.int32)
            )
...
class TestDtypeHandling:
    ...
    def test_matmul_bfloat16_input(self):
        """Test that bfloat16 input is accepted and preserves dtype (Req 1.7)."""
