...
This repository is maintained with **OpenSpec-first development** and is currently being stabilized for archive-ready maintenance.

## Before you change code

1. Check `openspec/changes/` for an active change.
2. Read the relevant change artifacts and `openspec/specs/<capability>/spec.md`.
3. If the requested work is non-trivial and no change exists, start with an OpenSpec proposal instead of editing immediately.

## Repository-specific priorities

- Keep the project positioned as a **hands-on, forward-only Triton FlashAttention implementation**.
- Keep README, bilingual docs, GitHub Pages, and GitHub About metadata aligned.
- Prefer removing stale docs and redundant workflow clutter over adding more scaffolding.
- Keep CI and hooks lightweight and high-signal.

## Editing rules

- Avoid generic contributor/process boilerplate.
- Avoid introducing new infrastructure unless it reduces real maintenance pain in this repo.
- Update nearby specs/docs/tests when behavior or project messaging changes.
- For broad or risky changes, use a review pass before merge.
