...
from utils.benchmark import BenchmarkRunner, BenchmarkResult, calculate_attention_flops, benchmark_fn
...
def run_memory_scaling_test(
    ...
    print(f"{'Seq Len':<10} | {'PyTorch (MB)':<15} | {'Flash (MB)':<15} | {'Ratio':<10} | {'Scaling':<15}")
    print("-" * 70)
    ...
    for seq_len in seq_lengths:
        ...
        try:
            ...
            print(f"{seq_len:<10} | {pytorch_mem:<15.1f} | {flash_mem:<15.1f} | {ratio:<10.2f}x | {scaling_info:<15}")
...
def main():
    ...
    for seq_len in args.seq_lengths:
        ...
        if pytorch_result and flash_result:
            ...
            mem_reduction = pytorch_result.memory_mb / flash_result.memory_mb if flash_result.memory_mb > 0 else float('inf')
            
            print(f"  Seq={seq_len}:")
            print(f"    Speedup: {speedup:.2f}x {'faster' if speedup > 1 else 'slower'}")
            print(f"    Memory:  {mem_reduction:.2f}x reduction")
            print(f"    PyTorch: {pytorch_result.tflops:.1f} TFLOPS, {pytorch_result.memory_mb:.1f} MB")
