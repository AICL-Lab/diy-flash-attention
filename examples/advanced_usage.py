...
def demo_causal_masking():
    ...
    # 前半部分应该相同
    first_half_diff = (out_causal[:, :, :seq_len//2, :] - out_causal_modified[:, :, :seq_len//2, :]).abs().max()
    print(f"  修改后半部分 K 后，前半部分输出差异: {first_half_diff.item():.2e}")
    print(f"  ✓ 因果性验证通过!" if first_half_diff < 1e-3 else "  ✗ 因果性验证失败!")
...
def demo_batch_performance():
    ...
    for batch in [1, 2, 4, 8, 16]:
        ...
        ms = measure_time(lambda: flash_attention(q, k, v, causal=True), warmup=5, repeat=50)
