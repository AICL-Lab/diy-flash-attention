...
def visualize_tiling(M: int, N: int, K: int, block_m: int, block_n: int, block_k: int):
    ...
    if total_sram > 192 * 1024:
        print("  ⚠️  Warning: Block size may exceed SRAM capacity!")
    else:
        print("  ✓ Block size fits in SRAM")
    ...
    print("\n  Typical GPU specs:")
    print("    A100: ~312 TFLOPS (FP16), ~2 TB/s HBM bandwidth")
    print("    H100: ~990 TFLOPS (FP16), ~3.35 TB/s HBM bandwidth")
