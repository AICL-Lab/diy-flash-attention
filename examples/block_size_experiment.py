...
def experiment_block_sizes():
    ...
    if results:
        ...
        print("\n💡 Key Insights:")
        print("   - Block size significantly affects performance")
        print("   - Larger blocks often better for data reuse")
        print("   - But too large can reduce parallelism")
        print("   - Optimal depends on matrix size and GPU architecture")
    ...
    if results:
        ...
        if tflops_auto > best_manual:
            print(
                f"   Autotune is {(tflops_auto / best_manual - 1) * 100:.1f}% better than best manual config"
            )
        else:
            print(
                f"   Best manual config is {(best_manual / tflops_auto - 1) * 100:.1f}% better than autotune!"
            )
